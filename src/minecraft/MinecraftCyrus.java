...
public class MinecraftCyrus extends WebObject {
    ...
    public void registerWorld(String n, MinecraftWorld w){ newWorld(n,w); }

    static public void onTick(){ if(self!=null) self.doOnTick(); }

    int initialDelay=10;
    public void doOnTick(){
        if(initialDelay >0){ initialDelay--; return; }
    ...
    private void newWorld(final String worldname, final MinecraftWorld w){
        new Evaluator(this){ public void evaluate(){
            if(contentAllContains("worlds:name",worldname)) return;
            contentListAdd("worlds", w.uid);
        }};
    }
    ...
    public void evaluate(){
        if(contentIsOrListContains("is","site") && first){
            first=false;
            setup();
        } else
    ...
    private void handleAlertedForms(){
